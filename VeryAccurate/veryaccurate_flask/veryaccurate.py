            ...
            if not self.q.empty():
                ...
                if seach_result:
                    ...
                    for selector in mylist:
                        ...
                        # 实际起飞
                        FlightDeptime_img_link = selector.xpath('div[@class="li_com"]/span[6]/img/@src')

                        #计划到达
                        FlightArrtimePlan = selector.xpath('div[@class="li_com"]/span[5]/@aplan')[0]

                        # 实际到达
                        FlightArrtime_img_link = selector.xpath('div[@class="li_com"]/span[3]/img/@src')

                        if FlightDeptime_img_link and FlightArrtime_img_link:
                            FlightDeptime_img_link = urljoin(start_response.url, FlightDeptime_img_link[0])
                            time1 = self.parse_time(FlightDeptime_img_link,cookie)
                            _time1 = time1.split(":")[0] if time1.split(":")[0][0] else time1.split(":")[0].replace('0', '')

                            FlightArrtime_img_link = urljoin(start_response.url, FlightArrtime_img_link[0])
